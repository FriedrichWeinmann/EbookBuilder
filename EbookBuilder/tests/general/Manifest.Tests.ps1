Describe "Validating the module manifest" {
	...
	$manifest = ((Get-Content "$moduleRoot\EbookBuilder.psd1") -join "`n") | Invoke-Expression
	[version]$moduleVersion = Get-Item "$moduleRoot\EbookBuilder.psm1" | Select-String -Pattern '\$script:ModuleVersion = "(.*?)"' | ForEach-Object { $_.Matches[0].Groups[1].Value }
